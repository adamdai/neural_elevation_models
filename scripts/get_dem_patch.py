...
from nemo.util.plotting import plot_3d_points, plot_surface
...
patch_min = np.array(
    [center_idx[0] - PATCH_WIDTH_M // 2, center_idx[1] - PATCH_WIDTH_M // 2], dtype=int
)
patch_max = np.array(
    [center_idx[0] + PATCH_WIDTH_M // 2, center_idx[1] + PATCH_WIDTH_M // 2], dtype=int
)
...
if PLOT:
    ...
    plt.plot(
        [patch_min[0], patch_max[0], patch_max[0], patch_min[0], patch_min[0]],
        [patch_min[1], patch_min[1], patch_max[1], patch_max[1], patch_min[1]],
        color="red",
    )
    plt.show()
